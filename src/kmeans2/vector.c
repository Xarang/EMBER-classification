...
void mask_init(float *data, unsigned nb_vec, unsigned vec_dim)
{
    ...
    for (unsigned i = 0; i < MASK_SIZE; i++)
    {
        ...
        max[i] = 1;//max_absolute_values[deviations[i].index];
        weights[i] = 1;//log(MASK_SIZE) - log(i + 1) + 0.001;
    }
...
/*
** exactly the same thing as distance, except sqrt is not computed at the end
*/
